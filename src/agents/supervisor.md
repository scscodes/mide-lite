...
### Phase 1 – Analysis
Read `src/workflows/index.xml` and `src/memory.xml`.
...
Update `src/memory.xml` with the result of the workflow.
...
## Orchestration Rules
- Always check `src/memory.xml` before starting a new workflow to avoid context switching.
