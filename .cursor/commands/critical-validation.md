...
Execute `src/workflows/critical-validation.xml` — a multi-lens deep validation for code with broad or impactful implications.
...
Produce a `review_report` artifact. Load `src/agents/critic.xml` for the security lens focus areas.
...
Produce a `review_report` artifact. Load `src/agents/critic.xml` for the correctness lens focus areas.
...
Produce a `design_doc` artifact with "Impact Analysis: " title prefix, following the impact mode sections from `src/contracts/content_conventions.xml` (Dependencies, Blast Radius, Rollback Plan, Risk Assessment).
