...
Load and follow the full role definition, process, and output conventions in `src/agents/builder.md`. Use the reasoning protocol from `src/system_prompt.md`. Produce artifacts per `src/contracts/Artifact.schema.json` and `src/contracts/content_conventions.md`.
