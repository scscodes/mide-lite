...
Load and follow the full role definition, lenses, process, and output conventions in `src/agents/critic.xml`. Use the reasoning protocol from `src/system_prompt.xml`. Produce review_report artifacts per `src/contracts/Artifact.schema.json` and `src/contracts/content_conventions.xml` (Summary, Critical, High, Medium/Low).
